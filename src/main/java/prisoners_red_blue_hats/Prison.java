...
//
            names.add(name);
//
            prisoners.add(new Prisoner(HatColor.randomColor(), name));
        }
//
        return prisoners;
    }
...
//            */
    public long run() {
        ArrayList<HatColor> observation = new ArrayList<>(prisoners.stream().map(Prisoner::getHatColor).toList());

        ArrayList<HatColor> history = new ArrayList<>();

        Executioner executioner = new Executioner();
        for (int i = 0; i < size; i++) {
            HatColor answer = executioner.ask(prisoners.get(i), observation, history);
            observation.remove(0);
            history.add(answer);
        }
//
        return prisoners.stream().filter(Prisoner::isAlive).count();
